        ...
        for word in ngrams:
            ...
            for wd in words:
                if wd in vocab.token2id.keys() and tf[0][vocab.token2id[wd]] > 0 and wd in model.wv:
                    new_vec = model.wv[wd] /np.linalg.norm(model.wv[wd], 2) * np.log(tf[0][vocab.token2id[wd]]+1) * (1+np.log(len(corpus)) - np.log(idf[vocab.token2id[wd]]))
                    vec += new_vec
                    n += 1
            if n > 0:
